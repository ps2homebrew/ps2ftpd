        ...
        case FS_IODEVICE: {
            ...
            if ((pContext->m_kFile.device->type & 0xf0000000) != IOP_DT_FSEXT) {
                ...
                if (pContext->m_kFile.device->ops->dread(&(pContext->m_kFile), &ent) > 0) {
                    ...
                    pInfo->m_eType = FIO_SO_ISDIR(ent.stat.mode) ? FT_DIRECTORY : FIO_SO_ISLNK(ent.stat.mode) ? FT_LINK :
                                                                                                                FT_FILE;
                ...
                if (pContext->m_kFile.device->ops->dread(&(pContext->m_kFile), &ent) > 0) {
                    ...
                    pInfo->m_iSize = ent.stat.size;
                    pInfo->m_eType = FIO_S_ISDIR(ent.stat.mode) ? FT_DIRECTORY : FIO_S_ISLNK(ent.stat.mode) ? FT_LINK :
                                                                                                              FT_FILE;
